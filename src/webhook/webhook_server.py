...
def strava_publishing_enabled():
    """Strava auto-publish is on only when a token is configured and it isn't
    explicitly disabled via STRAVA_AUTO_PUBLISH=false."""
    if os.getenv('STRAVA_AUTO_PUBLISH', 'true').lower() == 'false':
        return False
    return bool(os.getenv('STRAVA_ACCESS_TOKEN'))
...
"""
        ...
        # Auto-publish to Strava (skipped when no token is configured)
        strava_activities = []
        if strava_publishing_enabled():
            ...
            logger.info("Strava publishing disabled or not configured - skipping")
            message = 'Email ingested (Strava publishing skipped)'
